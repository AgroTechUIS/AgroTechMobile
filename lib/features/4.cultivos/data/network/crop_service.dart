...
class CropService {
  ...
  Future<HttpResponseModel> saveCrop({required CropResponseModel cropModel}) async {
    return clienthttp.post(url: '${RouteService.routeService}/crops', body: cropModel.toJson());
  }

  Future<HttpResponseModel> getCrops({required int idEmpresa}) async {
    final response = await clienthttp.get(url: '${RouteService.routeService}/api/crop/$idEmpresa');

    return response;
  }

  Future<HttpResponseModel> getPlants() async {
    final response = await clienthttp.get(url: '${RouteService.routeService}/api/plantas/all');

    return response;
  }

  Future<HttpResponseModel> saveCropData(Map<String, dynamic> cropData) async {
    final response = await clienthttp.post(url: '${RouteService.routeService}/api/crop', body: cropData);
    return response;
  }

  Future<HttpResponseModel> updateCropData(Map<String, dynamic> cropData) async {
  ...
  Future<HttpDeleteModel> deleteCropData(int? idCrop) async {
    final response = await clienthttp.delete(url: '${RouteService.routeService}/api/crop/$idCrop');
    return response;
  }
}
