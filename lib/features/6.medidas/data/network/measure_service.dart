...
class MeasureService {
  ...
  Future<HttpResponseModel> saveMeasure(
      Map<String, dynamic> measureModel) async {
    return clienthttp.post(
        url: '${RouteService.routeService}/api/variable-value',
        body: measureModel);
  }

  Future<HttpResponseModel> getMeasures(int idVariable) async {
    return clienthttp.get(
        url: '${RouteService.routeService}/api/variable-value/$idVariable');
  }

  Future<HttpResponseModel> updateMeasureData(
      Map<String, dynamic> measureData) async {
    final response = await clienthttp.put(
      url:
          '${RouteService.routeService}/api/variable-value', // Ruta sin el cropId
      body: measureData,
    );
    return response;
  }
  ...
  Future<HttpDeleteModel> deleteMeasureData(int? idMeasure) async {
    final response = await clienthttp.delete(
        url: '${RouteService.routeService}/api/variable-value/$idMeasure');
    return response;
  }
}
